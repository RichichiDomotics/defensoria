...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.NumerarioWSDTO;
...
public class NumerarioTransformer {
	...
	public static NumerarioWSDTO transformarNumerarioDTO(NumerarioDTO src){
		NumerarioWSDTO dest = null;
		if(src != null){
			dest = new NumerarioWSDTO();
			dest.setFechaDeposito(src.getFechaDeposito().toString());
			dest.setCtaTesoreria(src.getCtaTesoreria());
			dest.setCantidad(src.getCantidad());
			dest.setMoneda(src.getMoneda());
			dest.setEsActivo(src.getEsActivo());					
