...
/**
 * Implementacion para enviar la replica del caso a las dem&aacute;s intituciones.
 * 
 * @version 1.0
 * @author vaguirre
 * 
...
public class EnviarReplicaCasoServiceImpl implements EnviarReplicaCasoService {
     ...
     */
    @Override
    public void enviarReplicaCaso(ExpedienteDTO expdienteConCaso)throws NSJPNegocioException {

        Expediente exp 			= this.expDao.read(expdienteConCaso.getExpedienteId());
        ExpedienteDTO expParam 	= ExpedienteTransformer.transformarExpedienteBasico(exp);
        expParam.getCasoDTO().setFechaApertura(exp.getCaso().getFechaApertura());

        List<Involucrado> tempo = new ArrayList<Involucrado>();

        tempo.addAll(this.involucradoDao.obtenerInvByIdExpAndCalidad(expdienteConCaso.getExpedienteId(),Calidades.VICTIMA_PERSONA.getValorId(), null));
        tempo.addAll(this.involucradoDao.obtenerInvByIdExpAndCalidad(expdienteConCaso.getExpedienteId(),Calidades.PROBABLE_RESPONSABLE_PERSONA.getValorId(), null));
        tempo.addAll(this.involucradoDao.obtenerInvByIdExpAndCalidad(expdienteConCaso.getExpedienteId(),Calidades.PROBABLE_RESPONSABLE_ORGANIZACION.getValorId(), null));
        tempo.addAll(this.involucradoDao.obtenerInvByIdExpAndCalidad(expdienteConCaso.getExpedienteId(),Calidades.DENUNCIANTE.getValorId(), null));
        ...
        List<ConfInstitucion> instituciones = this.institucionDao.consultarDemasIntituciones();
        for (ConfInstitucion inst : instituciones) {
            try {
                ...
                logger.error("No se pudo replicar la informaci&oacute;n del caso en "+ inst.getNombreInst() + ". Se obtuvo el siguiente error: " + e.getMessage(), e);
            }
        }
    }
