...
public class ConsultarCarpetaDeInvestigacionServiceImpl implements
	 ...
	 */
	@Override
	public ExpedienteDTO consultarCarpetaDeInvestigacion(String numeroGeneralCaso ) throws NSJPNegocioException {
		...
		if(expedientes==null || expedientes.isEmpty() || expedientes.get(0)==null || expedientes.get(0).getExpedienteId()== null || expedientes.get(0).getNumeroExpedienteId()==null ){
			throw new NSJPNegocioException(CodigoError.INFORMACION_PARAMETROS_ERRONEA);
		}
		...
		if(carpetaInvestigacion.getDelitos()!= null && !carpetaInvestigacion.getDelitos().isEmpty()){
			...
			for (DelitoDTO delitoDTO : delitosV) {
				if(delitoDTO.getDelitoId()!= null){
					...
					logger.info(" delito: "+  delito.getEsPrincipal() + " EsProbable: "+  delito.getEsProbable()+ " Expediente: "+  delito.getExpediente());
