...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.EquipoComputoWSDTO;
...
public class EquipoComputoTransformer {
	...
	public static EquipoComputoWSDTO transformarEquipoComputoDTO(EquipoComputoDTO src){
		EquipoComputoWSDTO dest = null;
		
		if(src != null){
			dest = new EquipoComputoWSDTO();
			...
			if(src.getTipoEquipo() != null){
				dest.setTipoEquipo(src.getTipoEquipo().getIdCampo());
			}
			dest.setNoSerie(src.getNoSerie());
			dest.setModelo(src.getModelo());
			dest.setEsActivo(src.getEsActivo());		
