...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.JoyaWSDTO;
...
public class JoyaTransformer {
	...
	public static JoyaWSDTO transformarJoyaDTO(JoyaDTO src){

		JoyaWSDTO dest =  null;
		if(src != null){
			dest = new JoyaWSDTO();
			if(src.getTipoJoya() != null){
				dest.setTipoJoya(src.getTipoJoya().getIdCampo());
			}
			dest.setCantidad(src.getCantidad());
			dest.setMaterial(src.getMaterial());
			dest.setEsActivo(src.getEsActivo());			
