...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.ArmaWSDTO;
...
//                }
	...
	public static ArmaWSDTO  transformarArmaDTO(ArmaDTO src){
		ArmaWSDTO dest = null;
		if(src != null){
			dest = new ArmaWSDTO();
			if(src.getMarca() != null){
			...
			if(src.getTipoArma() != null){
				dest.setTipoArma(src.getTipoArma().getIdCampo());
			}
			dest.setModelo(src.getModelo());
			dest.setMatricula(src.getMatricula());
			dest.setCalibre(src.getCalibre());
