...
public class AvisoDetencionServiceImpl implements AvisoDetencionService {
	...
	@Override
	public Boolean tieneInvolucradoAvisoDetencion(Long idInvolucrado){
		return avisoDetencionDAO.involucradoTieneSolicitudDefensor(idInvolucrado);
	}
	
	@Override
	public Boolean tieneInvolucradoDenunciaGenerada(Long idInvolucrado)throws NSJPNegocioException{
		return avisoDetencionDAO.involucradoTieneDenuncia(idInvolucrado);
	}
	
	@Override
	public Boolean tieneAvisoDetencion(Long idInvolucrado)throws NSJPNegocioException{
		return avisoDetencionDAO.tieneAvisoDetencion(idInvolucrado);
	}	
	...
	@Override
	public AvisoDetencionDTO obtenerAvisoDetencionCoordinadorDef(Long expedienteId) throws NSJPNegocioException{
		if (logger.isDebugEnabled()){
			logger.debug("/**** SERVICIO PARA COLOCAR EL ESTATUS DEL AVISO EN ATENDIDO ****/");
		}
		...
		AvisoDetencionDTO avisoDetCoor  	= new AvisoDetencionDTO();
		List<AvisoDetencion> avisoDet 		= avisoDetencionDAO.consultarAvisosDetencionPorExpediente(expedienteId);
		/// AvisoDetencionDTO avisoDetDTO = AvisoDetencionTransformer.transformarAvisoDetencion(avisoDetencion);
		avisoDetCoor						= AvisoDetencionTransformer.transformarAvisoDetencion(avisoDet.get(0));
		Detencion detencion					= detencionDAO.consultarDetencion(avisoDetCoor.getDetencion().getDetencionId());
		avisoDetCoor.setDetencion(DetencionTransformer.transformarDetencion(detencion));	
		return avisoDetCoor;
	}
