...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.AnimalWSDTO;
...
public class AnimalTransformer {
	...
	public static AnimalWSDTO transformarAnimalDTO(AnimalDTO src){
		AnimalWSDTO dest = null;
		if(src != null){
			dest = new AnimalWSDTO();
			if(src.getTipoAnimal()!=null){
				dest.setTipoAnimal(src.getTipoAnimal().getIdCampo());
			}
			dest.setEstadoAnimal(src.getEstadoAnimal());
			dest.setRazaAnimal(src.getRazaAnimal());
			dest.setEsActivo(src.getEsActivo());		
