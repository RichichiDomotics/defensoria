...
public class ConsultarAudienciaServiceImpl implements ConsultarAudienciaService {
    ...
    public List<AudienciaDTO> buscarAudiencias(FiltroAudienciaDTO filtro)
    	...
    	if(filtro.getEstatusSolicitud() != null){
    		estatusSolicitud=filtro.getEstatusSolicitud();
    	}else{
    		filtro.setEstatusSolicitud(estatusSolicitud);
    	}

    	//final Long usrId = filtro.getUsuario() != null ? filtro.getUsuario().getIdUsuario() : null;
        final Long usrId = filtro.getUsuario().getFuncionario().getClaveFuncionario() != null ? filtro.getUsuario().getFuncionario().getClaveFuncionario() : null;
        
        /*
        * Enable IT : obteniendo el rol, para mandarlo como parametro
        * */
       final String rol = filtro.getUsuario().getRolACtivo().getRol().getNombreRol() != null ? filtro.getUsuario().getRolACtivo().getRol().getNombreRol() : null;
        ...
        if (filtro.getNumeroExpediente()!=null) {
        	...
        	fromBd = this.audDao.buscarAudiencias(usrId,
                    ...
                    filtro.getFechaFinal(), filtro.isDiaCompleto(), filtro.getTipoAudiencia(), filtro.isCausa(),discriminanteId, filtro.getIdDistritoFiltroAudiencias(),filtro.getEstatusAudiencia(),rol,filtro.getEstatusSolicitud());
        }               
	...
	/*
	 * Metodo para consultar audiencias programadas en determinadas fechas y por usuario duenio de expediente
	 * */
	public List<AudienciaDTO> consultarAudienciasByFechasyFuncionario(
			AudienciaDTO audiencia, Long claveFuncionario)
			throws NSJPNegocioException {

		List<Audiencia> audiencias = audDao.consultarAudienciasPorUsuario(
				claveFuncionario, audiencia.getFechaFiltroInicio(),
				audiencia.getFechaFiltroFin());

		List<AudienciaDTO> audRetorno = new ArrayList<AudienciaDTO>();
		for (Audiencia audiencia1 : audiencias) {
			audRetorno.add(AudienciaTransformer.transformarDTO(audiencia1));
		}
