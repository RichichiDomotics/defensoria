...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.SustanciaWSDTO;
...
public class SustanciaTransformer {
	...
	public static SustanciaWSDTO transformarSustanciaDTO(SustanciaDTO src){
		SustanciaWSDTO dest = null;
		if(src != null){
			dest = new SustanciaWSDTO();
			if(src.getTipoSustancia() != null){
				dest.setTipoSustancia(src.getTipoSustancia().getIdCampo());
			}
			if(src.getEmpaque() != null){
				dest.setEmpaque(src.getEmpaque().getIdCampo());
			}
			if(src.getUnidadMedida() != null){
				dest.setUnidadMedida(src.getUnidadMedida().getIdCampo());
			}
			dest.setCantidad(src.getCantidad());
			dest.setEsActivo(src.getEsActivo());
		}
