...
import mx.gob.segob.nsjp.dto.audiencia.CausaDTO;
import mx.gob.segob.nsjp.dto.audiencia.SalaAudienciaDTO;
import mx.gob.segob.nsjp.dto.audiencia.SalaTemporalDTO;
import mx.gob.segob.nsjp.dto.catalogo.CatAudienciaDTO;
import mx.gob.segob.nsjp.dto.catalogo.CatDiscriminanteDTO;
import mx.gob.segob.nsjp.dto.catalogo.CatJuzgadoDTO;
...
//					.getNumeroExpediente());
		...
		dto.setTipoAudiencia(new ValorDTO(aud.getTipo().getValorId(),aud.getTipo().getValor()));
		...
		if(aud.getEstatus()!=null && aud.getEstatus().getValor() != null){
			ValorDTO val = new ValorDTO();
			val.setValor(aud.getEstatus().getValor());
			val.setIdCampo(aud.getEstatus().getValorId());
			dto.setEstatusAudiencia(val);
		}
		...
		if(aud.getSalaAudiencia()!= null){
			...
			if(aud.getSalaAudiencia().getCatDiscriminante() != null){
				CatDiscriminanteDTO catDisc = new CatDiscriminanteDTO();
				catDisc.setCatDiscriminanteId(aud.getSalaAudiencia().getCatDiscriminante().getCatDiscriminanteId());
				catDisc.setNombre(aud.getSalaAudiencia().getCatDiscriminante().getNombre());
				salaAudiencia.setCatDiscriminanteDTO(catDisc);
			}
			if (aud.getSalaAudiencia().getCatJuzgado() != null
					&& aud.getSalaAudiencia().getCatJuzgado().getCatJuzgadoId() != 0) {
				CatJuzgadoDTO catJuzgado = new CatJuzgadoDTO();
				catJuzgado.setCatJuzgadoId(aud.getSalaAudiencia().getCatJuzgado().getCatJuzgadoId());
				catJuzgado.setcDescripcion(aud.getSalaAudiencia().getCatJuzgado().getcDescripcion());
				catJuzgado.setcDireccion(aud.getSalaAudiencia().getCatJuzgado().getcDireccion());
				salaAudiencia.setCatJuzgadoDTO(catJuzgado);
			}
		...
		if(aud.getCausa() != null && aud.getCausa().getCausaId() != null){
			CausaDTO causa = new CausaDTO();
			causa.setCausaId(aud.getCausa().getCausaId());
			causa.setcCausaPenalIdPJ(aud.getCausa().getcCausaPenalIdPJ());
			dto.setCausadto(causa);
		}
		
		if(aud.getCatAudiencia() != null && aud.getCatAudiencia().getCatAudienciaId() != null) {
			CatAudienciaDTO catAudiencia = new CatAudienciaDTO();
			catAudiencia.setCatAudienciaId(aud.getCatAudiencia().getCatAudienciaId());
			catAudiencia.setDescripcion(aud.getCatAudiencia().getDescripcion());
			catAudiencia.setbActivo(aud.getCatAudiencia().getbActivo());
			dto.setCatAudienciadto(catAudiencia);
		}
	...
	public static AudienciaDefensoriaWSDTO transformarWSDTOBasicoConCaracter(Audiencia entity) {
        AudienciaDefensoriaWSDTO wsdto = new AudienciaDefensoriaWSDTO();
        ...
        if(entity.getEsPublica() != null){
        	wsdto.setCaracter(entity.getEsPublica() == true ? "Pública":"Privada");
		}else{
