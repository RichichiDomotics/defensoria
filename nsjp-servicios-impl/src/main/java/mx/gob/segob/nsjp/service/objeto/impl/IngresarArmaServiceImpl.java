...
public class IngresarArmaServiceImpl implements IngresarArmaService {
    ...
    private Long guardar(ArmaDTO armaDto) throws NSJPNegocioException {
        ...
        if (logger.isDebugEnabled()) {
            System.out.println("Inicia almacenamiento del Arma");
        }
        if (armaDto == null) {
            System.out.println("El arma es requerido para el guardado");
        ...
        System.out.println("Antes del if descripcion estado fisico");
        if (calidadDTO != null && calidadDTO.getDescripcionEstadoFisico() != null) {
            System.out.println("Estado fisico: "+calidadDTO.getDescripcionEstadoFisico());
            cal.setDescripcionEstadoFisico(calidadDTO.getDescripcionEstadoFisico());
        }
        System.out.println("Antes del if Calidades");
        if (calidadDTO != null && calidadDTO.getCalidades() != null) {
            System.out.println("Calidad ID: "+calidadDTO.getCalidades().getValorId());
            cal.setTipoCalidad(new Valor(calidadDTO.getCalidades().getValorId()));
        }
        idCalidad = this.calidadDAO.create(cal);
        System.out.println("Insertando Calidad " + idCalidad);
        cal.setCalidadId(idCalidad);
        System.out.println("Calidad: "+cal);
        arma.setCalidad(cal);
        System.out.println("Valor objeto: "+Objetos.ARMA.getValorId());
        arma.setValorByTipoObjetoVal(new Valor(Objetos.ARMA.getValorId()));
        System.out.println("Elemento: "+Elementos.OBJETO.getValorId());
        ...
        System.out.println("Se genero el arma con id: " + idArma);
        return idArma;
    }
