...
//					if (detencionDTO.getInvolucradoDTO().getSituacionImpDTO().getSituacionImputadoId() != null)
//						involucradoDTORetorno.setSituacionImpDTO(detencionDTO.getInvolucradoDTO().getSituacionImpDTO());
	...
	public List<InvolucradoDTO> consultarInvolucradoExpediente(
		...
		for (Involucrado involucrado : involucrados) {
			invDTO = InvolucradoTransformer.transformarInvolucradoBasico(involucrado);
			invDTO.setExpedienteDTO(expedienteDTO);

			ConstructorCamposFaltantes.crearNombresDemograficosParaDTO(relacionDAO, organizacionDAO,
					involucrado, invDTO);

			involucradosDTO.add(invDTO);
		}
		return involucradosDTO;
	}
	...
	public List<InvolucradoDTO> consultarInvolucradoPorCalidadCaso(
		...
		for (Involucrado inv : involucrados) {
			InvolucradoDTO invDTO = InvolucradoTransformer.transformarInvolucrado(inv);
