...
import mx.gob.segob.nsjp.ws.cliente.avisodetencion.ObraArteWSDTO;
...
public class ObraArteTransformer {
	...
	public static ObraArteWSDTO transformarObraArteDTO(ObraArteDTO src){
		ObraArteWSDTO dest = null;
		if(src != null){
			dest = new ObraArteWSDTO();
			if(src.getTipoObraArte() != null){
				dest.setTipoObraArte(src.getTipoObraArte().getIdCampo());
			}
			dest.setCantidad(src.getCantidad());
			dest.setEsActivo(src.getEsActivo());
		}
