...
public class IngresarObjetoServiceImpl implements IngresarObjetoService {
	...
	@Override
	public Long ingresarObjetoCarpetaInvestigacion(ObjetoDTO objetoDTO) throws NSJPNegocioException {
		Long idObjeto = 0L;
		if (objetoDTO == null){
		...
		if (objetoDTO instanceof AeronaveDTO) {
			idObjeto = ingresarAeronaveService.ingresarAeronave((AeronaveDTO) objetoDTO);
		}
		if (objetoDTO instanceof AnimalDTO) {
			idObjeto = ingresarAnimalService.ingresarAnimal((AnimalDTO) objetoDTO);
		}
		...
		if (objetoDTO instanceof DocumentoOficialDTO) {
			idObjeto = ingresarDocumentoOficialService.ingresarDocumentoOficial((DocumentoOficialDTO) objetoDTO);
		}
		if (objetoDTO instanceof EmbarcacionDTO) {
			idObjeto = ingresarEmbarcacionService.ingresarEmbarcacion((EmbarcacionDTO) objetoDTO);
		}
		if (objetoDTO instanceof EquipoComputoDTO) {
			idObjeto = ingresarEquipoDeComputoService.ingresarEquipoComputo((EquipoComputoDTO) objetoDTO);
		}
		if (objetoDTO instanceof ExplosivoDTO) {
			idObjeto = ingresarExplosivoService.ingresarExplosivo((ExplosivoDTO) objetoDTO);
		}
		...
		if (objetoDTO instanceof NumerarioDTO) {
			idObjeto = ingresarNumerarioService.ingresarNumerario((NumerarioDTO) objetoDTO);
		}
		if (objetoDTO instanceof ObraArteDTO) {
			idObjeto = ingresarObraArteService.ingresarObraArte((ObraArteDTO) objetoDTO);
		}
		if (objetoDTO instanceof SustanciaDTO) {
			idObjeto = ingresarSustanciaService.ingresarSustancia((SustanciaDTO) objetoDTO);
		}
		if (objetoDTO instanceof TelefoniaDTO) {
			idObjeto = ingresarTelefonoService.ingresarTelefono((TelefoniaDTO) objetoDTO);
		}
		if (objetoDTO instanceof VegetalDTO) {
			idObjeto = ingresarVegetalService.ingresarVegetalService((VegetalDTO) objetoDTO);
		}
		if (objetoDTO instanceof VehiculoDTO) {
			idObjeto = ingresarVehiculoService.ingresarVehiculo((VehiculoDTO) objetoDTO);
		}
		
		objetoDTO.setElementoId(idObjeto);
		if (objetoDTO.getEvidencia() != null) {
			EvidenciaDTO evidencia = objetoDTO.getEvidencia();
			CadenaDeCustodiaDTO cadena = objetoDTO.getEvidencia().getCadenaDeCustodia();
			if (cadena != null && cadena.getFolio() != null && !cadena.getFolio().equals("null")) {
				cadena = consultarCadenaDeCustodiaXFolioService.consultarCadenaDeCustodiaPorFolioExpediente(cadena.getFolio(), objetoDTO.getExpedienteDTO().getExpedienteId());
				if (cadena == null) {
					cadena = objetoDTO.getEvidencia().getCadenaDeCustodia();
					cadena = guardarCadenaCustodiaService.guardarCadenaCustodia(cadena,objetoDTO.getExpedienteDTO());
				}
				logger.info("CADENA DE CUSTOIA ENCONTRADA " + cadena.getFolio()+ " CON ID " + cadena.getCadenaDeCustodiaId());
				...
				evidencia.setEvidenciaId(guardarEvidenciaService.guardarEvidencia(evidencia));
				...
				for (EslabonDTO eslabon : eslabones) {
					if(eslabon.getDocumentoDTO() != null){
						...
						Long idDocumento = guardarDocumentoService.guardarDocumento(documento, new ExpedienteDTO(objetoDTO.getExpedienteDTO().getExpedienteId()), null);
						documento.setDocumentoId(idDocumento);
						eslabon.setDocumentoDTO(documento);
					}
					guardarEslabonService.guardarEslabonCarpetaInvestigacion(evidencia, eslabon);
				}
			}
		}
		...
		if (archivo != null) {
			...
			if (archivo.getArchivoDigitalId() == null) {
				archivo.setArchivoDigitalId(guardarArchivoDigitalService.guardarArchivoDigital(archivo));
			}
