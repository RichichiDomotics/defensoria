...
public class EvidenciaTransformer {
     ...
     * @return
     */
    public static EvidenciaDTO transformarEvidencia(Evidencia evidencia,boolean tranformarObjeto) {
        ...
        if (cadenaDeCustodia != null) {
            evidenciaDto.setCadenaDeCustodia(CadenaDeCustodiaTransformer.transformarCadenaDeCustodiaBasico(cadenaDeCustodia));
        }
        ...
        if (tranformarObjeto) {
            ...
            if (objeto != null) {
                evidenciaDto.setObjeto(ObjetoTransformer.transformarObjeto(objeto));
            }
        }
        ...
        if (funcionarioBaja != null) {
            evidenciaDto.setFuncionarioBaja(FuncionarioTransformer.transformarFuncionario(funcionario));
        }
        ...
        for (Eslabon eslabon : eslabones) {
            if (numEslabones == 0) {
                ...
                if (eslabon.getNumeroEslabon() > ultimoAsociado.getNumeroEslabon()) {
                    ultimoAsociado = eslabon;
                }
            }
            EslabonDTO eslabonDto = EslabonTransformer.transformarEslabon(eslabon);
            evidenciaDto.getEslabones().add(eslabonDto);
        }
        if (ultimoAsociado != null) {
            evidenciaDto.setUltimoEslabon(EslabonTransformer.transformarEslabon(ultimoAsociado));
        }
        evidenciaDto.setCantidad(0);
        if (evidencia.getEstatus() != null) {
            ValorDTO valorDTO = new ValorDTO(evidencia.getEstatus().getValorId());
            valorDTO.setValor(evidencia.getEstatus().getValor());
            evidenciaDto.setEstatus(valorDTO);

            if (evidencia.getEstatus().getValorId().equals(EstatusEvidencia.EN_ALMACEN.getValorId()))
                evidenciaDto.setCantidad(1);
        }
        if (evidencia.getDestinoLegal() != null) {
            ValorDTO valorDTO = new ValorDTO(evidencia.getDestinoLegal().getValorId());
