...
function ejecutarActuacionXMLResponse(xml){
	...
	var estatusId=$(xml).find('confActividadDocumentoDTO').find('estadoCambioExpediente').find('idCampo').text();
  	...
  	switch (parseInt(actividad)) {
		...
		/*ENABLE IT: Se agregan actuaciones para defensores */
		case 2803: //Recurso de apelacion
        ...
        break;       
		case 6455://Actividades.OFICIO_CIERRE_EXPEDIENTE	
			/***************************************************************************
			 * Enable IT Se agrega la enumeracion y la actuacion
			 * para poder realizar el cierre del expediente
	         * Dando de alta enumeracion para cierre de expediente en defensoria
	         * insert into Registro (Registro_id,bEsActivo) values ((select MAX(Registro_id)+1 from Registro ),1);
	         * insert into valor (Valor_id,cValor,CampoCatalogo_id,Registro_id)values (6455,'OFICIO CIERRE EXPEDIENTE',69,(select MAX(Registro_id) from Registro ))
	        *******************************************************************************/
			/*0 porque es un archivo digital*/
			actuacionAdjuntarOficio('0', formaID);
			
		break;
		/*ENABLE IT: Se agrega para que abra las actuaciones de CHI*/
        default:
        	asociarActividadExpediente(actividad);
			generarDocumentoSinCaso(formaID, titulo, usaeditor);
        break;
...
function actuacionAdjuntarDocumento(tipo, forma){
 ...
 /*********************************************
 * Enable IT cambio:****************************
 * Se crea la funcion para adjuntar el oficio,*
 * para cierre de expediente*******************
 **********************************************/
function actuacionAdjuntarOficio(tipo, forma){
	var accion = contextPath+"/adjuntarDocumento.do?";
	accion += "tipo="+tipo;
	accion += "&esconderArbol=0";
	if(!isEmpty(forma)){
	...
	accion += "&idExpediente="+actExpedienteId;
	accion += "&numeroExpedienteCadena="+ actNumeroExpedienteSt;
	openNewIframeWindow("iframewindowAdjuntarDocumento", accion, "Adjuntar Oficio", 300, 200, 650, 50);
	
	cambioEstatusExpediente();

}
/****************************************************************
 * ENable IT cambio:Cambiar el estatus del expediente a cerrado
 * ,manteniendolo en la etapa actual mandando una bandera la cual 
 * nos ayudara a cerrar el estatus del expediente
 *****************************************************************/
function cambioEstatusExpediente(){
	
	var param = "numeroExpedienteId="+actNumeroExpedienteId;
		param += "&etapaId="+actEtapaExpediente; 
		param += "&bandera=1";
		var accion = contextPath+'/cambiarEtapaDelExpediente.do';
		peticionSincronaAjaxXML(accion, param, parseActuacionesXMLResponse);
}
/********************FIN CAMBIOS ENABLE IT******************************************************/
...
function actuacionEnviarSolicitudAudiencia(){
	var accion = contextPath+"/solicitarAudienciaMich.jsp?";
	accion += "numeroExpediente=" + actNumeroExpedienteSt;
...
//				$("#teoriaDelCaso").show();
//				/*Enable IT agregando etapa de CONCILIACION_Y_MEDIACION en etapa tecnica al realizar cambio de etapa de expediente*/
//				if(tip == 2051 || tip == 2050){
//					$('#cmbEtapaExpediente').append('<option value="' + $(this).find('clave').text() + '">' + $(this).find('valor').text() + '</option>');
//				}
//				break;
...
function abreTeoria(){
	...
	openNewIframeWindow("iframewindowAbrirteoria", accion, "Teoria del caso", 1100, 530, 75, 30);	
}
