...
body,td,th {
	...
	$(document).ready(function() {
		...
		function defensaIntegracion() {
		   ...
		   /*Enable IT agregando a parametro valor de la etapa*/
			$.updateWindowContent("iframewindowVisorDefensor",'<iframe src="<%= request.getContextPath() %>/visorExpedienteDefensoria.do?idNumeroExpediente='+numExpediente+'&numeroCaso='+numeroCaso+'&idImputadoDef='+idImputadoDef+'" width="1140" height="400" />');
	...
	*/
	...
	,	center: {
		...
		,	onresize_end:			function () { $("#gridAvisosAsignacion").setGridWidth($("#mainContent").width() - 5, true);
												...
												$("#gridDetalleFrmPrincipalQuince").setGridWidth($("#mainContent").width() - 5, true);
		...
		function ocultaMuestraGridsAlertas(idGrid) {
			...
			$("#divGridDefensorCompartidos").css("display", "none");
			...
			switch(parseInt(idGrid)){
				...
				case 14:
					...
					break;	
				case 15:
					$("#divGridDefensorCompartidos").css("display", "block");
					gridDefensaCompartidos();
					break;					
			}
		...
		function restablecerPantallas(){
			...
			ajustarGridAlCentro($("#gridDetalleFrmPrincipalQuince"));
			...
			jQuery("#gridAvisosAsignacion").jqGrid({
				...
				colModel:[ 	{name:'folio',		index:'2041', align:'center', width:85},
							{name:'caso',		index:'2002', align:'center', width:180},
				           	...
				           	{name:'',	index:'2009', align:'center', width:150}, 
		...
		function gridDefensaIntegracionDefensoria() {
			jQuery("#gridDetalleFrmPrincipalTres").jqGrid({
				...
				datatype: "xml", 
				colNames:['Caso','Expediente','Imputado','IdImputado', 'Institución Origen', 'Fecha de Detención', 'Fecha de Designación'], 
				colModel:[ 	{name:'caso',index:'2002', width:15},
							{name:'expediente',index:'2003', width:15},
							{name:'imputado',index:'2009', width:30},
							{name:'idImputado',	index:'idImputado',	align:'center', width:50, hidden: true}, 
				...
				ondblClickRow: function(rowid) {
					...
					numeroExpediente=ret.expediente;
					numeroCaso=ret.caso;
					idImputadoDef=ret.idImputado.trim();
					defensaIntegracion();
				}
		...
		function gridDefensaCompartidos() {
			jQuery("#gridDetalleFrmPrincipalQuince").jqGrid({
				url : '<%= request.getContextPath()%>/consultarExpedientesCompartidosDefensor.do', 
				datatype: "xml", 
				colNames:['Caso','Expediente','Imputado','IdImputado', 'Institución Origen', 'Fecha de Detención', 'Fecha de Designación'], 
				colModel:[ 	{name:'caso',index:'2002', width:15},
							{name:'expediente',index:'2003', width:15},
				...
				ondblClickRow: function(rowid) {
					...
					numEtapa=<%=EtapasExpediente.INTEGRACION.getValorId()%>;
					valorEtapa="Integración";
					var ret = jQuery("#gridDetalleFrmPrincipalQuince").jqGrid('getRowData',rowid);
					numeroExpediente=ret.expediente;
					numeroCaso=ret.caso;
					idImputadoDef=ret.idImputado.trim();
					defensaIntegracion();
			...
			$("#gridDetalleFrmPrincipalQuince").trigger("reloadGrid");
			$("#gview_gridDetalleFrmPrincipalQuince .ui-jqgrid-bdiv").css('height', '450px');	
		}
		...
		function gridDefensaTecnicaDefensoria() {		
			jQuery("#gridDefensaTecnica").jqGrid({
				...
				datatype: "xml", 
				colNames:['Caso','Expediente','Imputado','IdImputado', 'Tipo Audiencia', 'Sala', 'Fecha Audiencia'], 
				colModel:[ 	{name:'caso',index:'2002', width:15},
							{name:'expediente',index:'2003', width:15},
							{name:'imputado',index:'2009', width:30}, 	
							{name:'idImputado',	index:'idImputado',	align:'center', width:50, hidden: true},							
		...
		function gridEjecucion() {
			jQuery("#gridEjecucion").jqGrid({
				...
				datatype: "xml", 
				colNames:['Caso','Expediente','Imputado','IdImputado', 'Tipo Audiencia', 'Sala', 'Fecha Audiencai'], 
				colModel:[ 	{name:'caso',index:'2002', width:15},
							{name:'expediente',index:'2003', width:15},
							{name:'imputado',index:'2009', width:30}, 	
							{name:'idImputado',	index:'idImputado',	align:'center', width:50, hidden: true},							
	...
	function gridAudiencias(){
		...
		if(gridAud==0){
    		jQuery("#gridAudiencias").jqGrid({ 
    			url: contextoPagina + '/consultarAudienciasDefensor.do?inicio='+fechaInicio+'&desarrolloJAVS='+desarrolloJAVS+'&fin='+fechaFin+'', 	   		
    			datatype: "xml", 
				colNames:['Audiencia','Caso','Carácter','Tipo de Audiencia','Fecha de Audiencia','Sala','Defensor de Oficio'], 
				colModel:[{name:'audiencia',	 index:'audiencia', 		width:60},
			          ...
			          {name:'defensor' ,index:'defensor', 		width:110}				         
					],			
				...
				ondblClickRow: function(rowid) {
					...
					//rowid = row.audiencia -> apuntan al mismo valor
			...
			jQuery("#gridAudiencias").jqGrid('setGridParam', {url: contextoPagina + '/consultarAudienciasDefensor.do?inicio='+fechaInicio+'&desarrolloJAVS='+desarrolloJAVS+'&fin='+fechaFin+'',
					datatype: "xml" });
		...
		<div id="accordionmenuprincipal">
			...
			<div>
				<ul id="seccion1tree" class="filetree">
					<li class="closed"  id="menuDefRestaurativa" onmousedown="ocultaMuestraGridsAlertas('2');" ><span class="folder">Otros</span>   
<!-- 					ANTES : Defensa en Terminación Anticipada del Procedimiento -->
						<ul></ul>
					</li>	
					<li class="closed"  id="menuDefIntegracion" onmousedown="ocultaMuestraGridsAlertas('3');" ><span class="folder" >Iniciados</span>
						<ul></ul>
					</li>
					<li class="closed" id="menuDefTecnica" onmousedown="ocultaMuestraGridsAlertas('5')" ><span class="folder"  >Concluido</span>
						<ul></ul>
					</li>	
					<li class="closed" id="menuDefEjecucion" onmousedown="ocultaMuestraGridsAlertas('7')"><span class="folder"  >En trámite</span>
						<ul></ul>
					</li>	
					<li class="closed" id="menuDefAsesoria" onmousedown="ocultaMuestraGridsAlertas('13')"><span class="folder"  >Asesoría legal</span>
						<ul></ul>
					</li>	
					<li class="closed" id="menuDefCompartidos" onmousedown="ocultaMuestraGridsAlertas('15')"><span class="folder"  >Compartidos</span>
						<ul></ul>
					</li>	
			...
			<%--Enable IT Oculando menu izquierdo AudienciaPP 
			...
			<div>
				<ul id="seccion2tree" class="filetree">
					...
					<!--Enable IT ocultando  Solicitudes generadas a periciales
					...
					<li class="closed" id="audioVideo" onclick="funcionalidadNoDisponible();"><span class="folder" >Audio y video</span>
					<ul></ul>
					</li>		
					<li class="closed" id="fechas" onclick="funcionalidadNoDisponible();"><span class="folder">Transcripciones</span>
					<ul></ul>
					</li> 
	...
	<%--Enable IT ocultando historico expedientes 		
	...
	<div id="mainContent">
		<div class="ui-layout-center">
			<div class="ui-layout-content">
				...
				<div id="divGridDefensorCompartidos">
					<table id="gridDetalleFrmPrincipalQuince"></table>
					<div id="pagerTres1"></div>
				</div>
